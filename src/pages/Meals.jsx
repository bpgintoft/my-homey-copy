...
import { Plus, ChefHat, ShoppingCart, Calendar, Clock, Users, Sparkles, Trash2, ExternalLink, BarChart3, Beef, Fish, Leaf, Drumstick, Star, ChevronDown, ChevronUp, Coffee, UtensilsCrossed, Utensils, Apple, IceCream, Search, X, TrendingUp } from 'lucide-react';
...
export default function Meals() {
  ...
  const filteredMeals = kidFriendlyMeals.filter(meal => {
    ...
    const searchMatch = !searchQuery || meal.name.toLowerCase().includes(searchQuery.toLowerCase()) || (meal.ingredients && meal.ingredients.some(ing => ing.toLowerCase().includes(searchQuery.toLowerCase())));
    return proteinMatch && typeMatch && ratingMatch && favoritesMatch && searchMatch;
  });
        ...
        <Tabs defaultValue="meals" className="space-y-6">
          ...
          <TabsContent value="meals" className="space-y-6">
            <h2 className="text-xl font-semibold text-gray-900">
              {filteredMeals.length} Family Favorites
            </h2>
                      ...
                      <div className="flex gap-3">
                        {meal.photo_url && (
                            <div className="w-16 h-16 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                              <img src={getThumbnailUrl(meal.photo_url)} alt={meal.name} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                            </div>
                          )}
          ...
          <TabsContent value="grocery">
            <GroceryTabContent
              groceries={groceries}
              showHiddenItems={showHiddenItems} setShowHiddenItems={setShowHiddenItems}
              showAddGrocery={showAddGrocery} setShowAddGrocery={setShowAddGrocery}
              newGroceryItem={newGroceryItem} setNewGroceryItem={setNewGroceryItem}
              addManualGroceryMutation={addManualGroceryMutation}
              clearAllGroceriesMutation={clearAllGroceriesMutation}
              togglePurchasedMutation={togglePurchasedMutation}
              updateGroceryNameMutation={updateGroceryNameMutation}
              updateGroceryQuantityMutation={updateGroceryQuantityMutation}
              deleteGroceryItemMutation={deleteGroceryItemMutation}
              focusedGroceryId={focusedGroceryId} setFocusedGroceryId={setFocusedGroceryId}
              editingGroceryNames={editingGroceryNames} setEditingGroceryNames={setEditingGroceryNames}
              expandedQtyId={expandedQtyId} setExpandedQtyId={setExpandedQtyId}
            />
            </TabsContent>
            </Tabs>
            </div>
