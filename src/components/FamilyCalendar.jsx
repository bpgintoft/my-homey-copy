...
export default function FamilyCalendar({ activities, initialEventId }) {
  ...
  const updateEventMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.setQueryData(['cachedCalendarEvents'], (old) => {
        ...
        const toISO = (dateStr, isAllDay) => {
          if (!dateStr) return dateStr;
          if (isAllDay) return dateStr; // Date-only format stays as-is
          // datetime-local is "YYYY-MM-DDTHH:MM", convert to ISO "YYYY-MM-DDTHH:MM:00Z"
          return dateStr + ':00Z';
        };
        ...
        return old.map(e => 
          e.google_event_id === editingEvent.id 
            ? {
                ...
                start: toISO(editingEvent.start, editingEvent.isAllDay),
                end: toISO(editingEvent.end, editingEvent.isAllDay),
              }
            : e
        );
      });
  ...
  return (
    <div className="-mx-6 px-6" data-calendar-root>
          ...
          {showSearch ? (
            ...
            >
              ...
              <Button
                ...
                onClick={() => setShowMonthlyView(prev => !prev)}
              >
                ...
                setTimeout(() => {
                  ...
                  if (element) {
                    ...
                    if (container) {
                      ...
                      const scrollTarget = element.offsetTop - 170;
      ...
      <div data-events-list className={`space-y-3 ${showMonthlyView ? 'hidden' : ''}`}>
        <AnimatePresence>
          {weekDays.map((day) => {
