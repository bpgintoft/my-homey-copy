...
const AVATAR_STYLE_PROMPT = (headshotUrl) =>
  `Convert this photo into a high-fidelity, flat-vector circular cartoon logo. Style: Clean, minimalist, modern 2D illustration. Palette: Use soft sage (#8DA399) and muted gold (#C5A059) for shading and accents. Background: 100% Transparent PNG. Reference photo: ${headshotUrl}`;
  ...
  try {
    ...
    for (const member of members) {
      ...
      const { url: generatedUrl } = await base44.asServiceRole.integrations.Core.GenerateImage({
        prompt: AVATAR_STYLE_PROMPT(member.original_headshot_url),
